...
// Tests for the GetNextBusinessDayMorningWithConfig function
func TestGetNextBusinessDayMorningWithConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "月曜8時_同日9時",
			currentTime: time.Date(2024, 1, 15, 8, 0, 0, 0, jst), // Monday 8:00 (JST)
			config:      config,
			expected:    time.Date(2024, 1, 15, 9, 0, 0, 0, jst), // Same day 9:00
		},
		{
			name:        "月曜12時_翌日9時",
			currentTime: time.Date(2024, 1, 15, 12, 0, 0, 0, jst), // Monday 12:00 (JST)
			config:      config,
			expected:    time.Date(2024, 1, 16, 9, 0, 0, 0, jst), // Next day 9:00
		},
		{
			name:        "金曜20時_翌週月曜9時",
			currentTime: time.Date(2024, 1, 19, 20, 0, 0, 0, jst), // Friday 20:00 (JST)
			config:      config,
			expected:    time.Date(2024, 1, 22, 9, 0, 0, 0, jst), // Next Monday 9:00
		},
		{
			name:        "土曜14時_月曜9時",
			currentTime: time.Date(2024, 1, 20, 14, 0, 0, 0, jst), // Saturday 14:00 (JST)
			config:      config,
			expected:    time.Date(2024, 1, 22, 9, 0, 0, 0, jst), // Monday 9:00
		},
		{
			name:        "日曜14時_月曜9時",
			currentTime: time.Date(2024, 1, 21, 14, 0, 0, 0, jst), // Sunday 14:00 (JST)
			config:      config,
			expected:    time.Date(2024, 1, 22, 9, 0, 0, 0, jst), // Monday 9:00
		},
		{
			...
			currentTime: time.Date(2024, 1, 15, 8, 0, 0, 0, jst), // Monday 8:00 (JST)
			config:      nil,
			expected:    time.Date(2024, 1, 15, 10, 0, 0, 0, jst), // Same day 10:00 (default)
		},
	}
...
// Test with different business hours configurations
func TestGetNextBusinessDayMorningWithConfig_DifferentBusinessHours(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			name:          "営業開始8時半",
			businessStart: "08:30",
			currentTime:   time.Date(2024, 1, 15, 8, 0, 0, 0, jst), // Monday 8:00
			expectedHour:  8,
			expectedMin:   30,
		},
		{
			name:          "営業開始11時",
			businessStart: "11:00",
			currentTime:   time.Date(2024, 1, 15, 10, 0, 0, 0, jst), // Monday 10:00
			expectedHour:  11,
			expectedMin:   0,
		},
		{
			name:          "営業開始7時",
			businessStart: "07:00",
			currentTime:   time.Date(2024, 1, 15, 6, 30, 0, 0, jst), // Monday 6:30
			expectedHour:  7,
			expectedMin:   0,
		},
	}
...
// Test with invalid business hours configurations
func TestGetNextBusinessDayMorningWithConfig_InvalidConfig(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify that the default (10:00) is used for invalid configurations
...
func TestGetNextBusinessDayMorningWithConfig_DifferentTimezones(t *testing.T) {
	...
	// UTC 23:00 corresponds to JST 8:00 (next day)
	// Jan 15 UTC 23:00 = Jan 16 JST 8:00
	// Since it is before business start at 9:00, the same day (Jan 16) at 9:00 should be returned
