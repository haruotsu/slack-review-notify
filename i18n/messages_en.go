...
• /slack-review-notify set-away @user on [YYYY-MM-DD] am - Set morning half-day leave
• /slack-review-notify set-away @user on [YYYY-MM-DD] pm - Set afternoon half-day leave
...
- Language: %s`,
	...
	// ==================== Command: map-user ====================
	// ==================== Modal: user_mapping ====================
	"modal.user_mapping.title":                         "User mapping",
	"modal.user_mapping.submit":                        "Save",
	"modal.user_mapping.close":                         "Close",
	"modal.user_mapping.header":                        "Link a GitHub user to a Slack user. PR authors are then excluded from reviewer selection.",
	"modal.user_mapping.existing_header":               "*Existing mappings*",
	"modal.user_mapping.legacy_flag":                   "(re-registration required)",
	"modal.user_mapping.github":                        "GitHub username",
	"modal.user_mapping.github.hint":                   "e.g. octocat",
	"modal.user_mapping.slack_user":                    "Slack user",
	"modal.user_mapping.slack_user.hint":               "Pick from the Slack user picker.",
	"modal.user_mapping.delete":                        "Delete this mapping",
	"modal.user_mapping.delete.hint":                   "When checked, the mapping for the GitHub username above is removed.",
	"modal.user_mapping.delete.option":                 "Delete",
	"modal.user_mapping.error.github_required":         "GitHub username is required.",
	"modal.user_mapping.error.slack_user_required":     "Pick a Slack user.",
	"modal.user_mapping.error.slack_user_not_resolved": "Could not resolve a Slack user ID.",
	"modal.user_mapping.open_button":                   "👥 User mapping",
	"modal.user_mapping.saved":                         "Saved mapping for GitHub user `%s`.",
	"modal.user_mapping.deleted":                       "Deleted mapping for GitHub user `%s`.",
	"modal.user_mapping.delete_not_found":              "No mapping found for GitHub user `%s`.",
	...
	"cmd.map_user.not_user_id":  "Could not resolve a Slack user ID (`%s`). Click the autocomplete suggestion in the slash command and send, or open the \"👥 User mapping\" modal via `/slack-review-notify help`.",
	...
	"cmd.set_away.half_day_expired":     "The specified half-day period has already passed.",
	"cmd.set_away.am_pm_requires_on":    "am/pm must be used with `on YYYY-MM-DD am` format.",
	...
	"common.leave_type.am":      "Morning half-day",
	"common.leave_type.pm":      "Afternoon half-day",

	// ==================== Settings Modal ====================
	"modal.title":                           "Review Notification Settings",
	"modal.submit":                          "Save",
	"modal.close":                           "Cancel",
	"modal.header":                          "*Edit review notification settings for this channel*\nEach label has independent settings. User mappings and away schedules are still managed via slash commands.",
	"modal.label_select":                    "Label to edit",
	"modal.label_select.create_new":         "+ Create new label…",
	"modal.new_label_name":                  "New label name",
	"modal.new_label_name.hint":             "Must match the label name applied to PRs in GitHub.",
	"modal.default_mention_user":            "Mention target (user)",
	"modal.default_mention_user.hint":       "Pick a user from Slack's picker. For anything else (subteam, free text) use the field below.",
	"modal.default_mention_text":            "Mention target (other)",
	"modal.default_mention_text.hint":       "Subteam ID (S…), @team-handle, or decorative free text. Used when the user picker above is empty. To actually ping a subteam, paste its S… ID here.",
	"modal.reviewer_list":                   "Reviewer pool (picker)",
	"modal.reviewer_list.hint":              "Pick individual reviewers from Slack's picker. Reviewers are drawn randomly from this pool.",
	"modal.reviewer_list_text":              "Reviewer pool (other, comma-separated)",
	"modal.reviewer_list_text.hint":         "Non-ID reviewers (legacy bare names, decorative entries). Merged with the picker selection at save time.",
	"modal.reminder_interval":               "Reminder interval before reviewer assignment (minutes)",
	"modal.reminder_interval.hint":          "Positive integer; how often to remind while no reviewer is assigned.",
	"modal.delete_config":                   "Delete this configuration",
	"modal.delete_config.option":            "Delete this label configuration (applied on save)",
	"modal.repository_list":                 "Target repositories (comma-separated)",
	"modal.repository_list.hint":            "e.g. owner/repo1, owner/repo2",
	"modal.reviewer_reminder_interval":      "Reminder interval after reviewer assignment (minutes)",
	"modal.reviewer_reminder_interval.hint": "Positive integer, e.g. 30. The pre-assignment reminder is set via the slash command.",
	"modal.business_hours_start":            "Business hours start",
	"modal.business_hours_end":              "Business hours end",
	"modal.timezone":                        "Timezone",
	"modal.timezone.hint":                   "e.g. Asia/Tokyo, UTC, America/New_York",
	"modal.required_approvals":              "Required approvals",
	"modal.required_approvals.hint":         "Integer between 1 and 10",
	"modal.language":                        "Language",
	"modal.is_active":                       "Notifications active",
	"modal.lang.ja":                         "日本語",
	"modal.lang.en":                         "English",
	"modal.active.true":                     "Active",
	"modal.active.false":                    "Inactive",
	"modal.open_button":                     "⚙️ Edit settings in UI",
	"modal.saved":                           "Saved settings for label \"%s\" 🎉",
	"modal.deleted":                         "Deleted settings for label \"%s\" 🗑️",
	"modal.open_button.new":                 "➕ Add a new label configuration",
	"modal.open_button.edit":                "⚙️ Edit `%s` settings",

	// ==================== Away Management Modal ====================
	"modal.away.open_button":              "🌴 Manage availability",
	"modal.away.title":                    "Manage availability",
	"modal.away.submit":                   "Save",
	"modal.away.close":                    "Close",
	"modal.away.header":                   "*Register / remove a user's leave*\nRegister: pick a user (dates and reason are optional) and save.\nRemove all: tick the box below to wipe every leave record for that user.",
	"modal.away.user":                     "Target user",
	...
	"modal.away.delete_all.hint":          "When checked, the inputs above are ignored and every leave record for the user is removed on save.",
	"modal.away.delete_all.option":        "Remove all leave records for this user",
	...
	"modal.away.leave_type":               "Leave type",
	"modal.away.leave_type.hint":          "Select morning or afternoon for half-day leave.",
	"modal.away.leave_type.full_day":      "Full day",
	"modal.away.leave_type.am":            "Morning half-day",
	"modal.away.leave_type.pm":            "Afternoon half-day",
	...
	"modal.away.error.half_day_multi_day": "Half-day leave must be for a single day. Start and end dates must match.",
	"modal.away.error.half_day_expired":   "The specified half-day period has already passed.",
