...
- **❌ CI失敗通知**: GitHub ActionsなどのCIが失敗すると、既存のレビュー依頼スレッドに自動通知
...
- Let me select individual eventsにチェックを入れて、以下のイベントを有効化:
  ...
  - **Check runs**: CI失敗を検知してスレッドに通知（CI失敗通知機能）
...
#### ❌ CI失敗通知
GitHub ActionsなどのCIが失敗すると、自動的にスレッドに通知されます：
- 失敗時: `❌ CI失敗: build が失敗しました`（詳細へのリンク付き）
- レビュー中のPRのみ対象（アクティブなレビュータスクがある場合）
- すべての失敗したジョブについて個別に通知
