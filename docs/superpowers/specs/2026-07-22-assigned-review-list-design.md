...
3. コマンド実行者のSlack User IDが `ApprovedBy` のカンマ区切り要素に完全一致しない。

`U123` が `U1234` に誤一致しないよう、最終判定では部分一致を使用しない。両方のフィールドに一致しても、同じタスクは1回だけ表示する。`completed`、`done`、`archived`、論理削除済みのタスク、他のユーザーだけに割り当てられたタスクは除外する。

全activeタスクをメモリへ読み込まないよう、DBでは `Reviewers` の部分一致または空白を除去した旧形式 `Reviewer` の一致で候補を前絞りする。`Reviewers` の部分一致による `U123` と `U1234` の誤一致は、Go側の完全一致判定で除外する。
...
- 想定外のステータスは、検索時に上記4種類だけへ限定するため表示されない。
...
5. 本人が承認済みのタスクと `paused`、`completed`、`done`、`archived`、論理削除済みタスクは除外される。
6. DB側で担当者候補を前絞りし、無関係なactiveタスクをメモリへ読み込まない。
7. 応答が明示的にephemeralであり、クリック可能なPR・チャンネルリンクとASCII emojiのステータス表示を含む。
8. PRタイトルの空白とSlackの特殊文字が安全に整形される。
9. 該当タスクがない場合にローカライズした正常メッセージを返す。
10. 51件以上一致した場合、古い50件だけを表示し、全件数と残件数を正しく表示する。
11. DB検索に失敗した場合にローカライズした警告メッセージを返す。
