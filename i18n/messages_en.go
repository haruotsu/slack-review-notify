...
- Language: %s`,
	...
	"cmd.set_away.invalid_time_range":          "Invalid time range format. Please use HH:MM-HH:MM format (e.g., 06:00-14:00). Write a reason as `reason ...`.",
	"cmd.set_away.from_time_after_until_time": "Start time must be before end time.",
	"cmd.set_away.time_range_needs_on":        "Times can only be given as `on YYYY-MM-DD HH:MM-HH:MM`. Use dates only with `from`/`until`, and write a reason as `reason ...`.",
	"cmd.set_away.unknown_token":              "Unrecognized input. Give dates as `on YYYY-MM-DD` or `from YYYY-MM-DD until YYYY-MM-DD`, and a reason as `reason ...`.",
	...
	"modal.away.all_day":          "All day",
	"modal.away.all_day.hint":     "Untick to show the start / end time pickers for hourly leave.",
	"modal.away.all_day.option":   "All day (no start / end time)",
	...
	"modal.away.error.date_required_for_time": "Pick a date as well when you set a time.",
	"modal.away.error.invalid_time":           "Time must be in HH:MM format.",
	"modal.away.error.until_before_from":      "End date must be on or after the start date.",
	"modal.away.error.until_time_before_from": "End time must be after start time.",
